...
def fuv_orientations_calc(b_l_quat, b_r_quat, t_r_quat, t_l_quat):
	...
	for i in range(len(b_l_quat)):
		quat_product_list_1.append(calc_funcs.hamilton_product(b_r_quat[i], b_l_quat[i]))
	for j in range(len(t_r_quat)):
		quat_product_list_2.append(calc_funcs.hamilton_product(t_r_quat[j], quat_product_list_1[j]))
	for k in range(len(t_l_quat)):
		quat_product_final.append(calc_funcs.hamilton_product(t_l_quat[k], quat_product_list_2[k]))
	return quat_product_final
...
def mighti_orientation_calc(bottom_left_vectors, bottom_right_vectors, top_right_vectors, top_left_vectors):
	"Outputs the orientation list for MIGHTI"
	b_l_quat = calc_funcs.vector_to_pure_quaternion(bottom_left_vectors)
	b_r_quat = calc_funcs.vector_to_pure_quaternion(bottom_right_vectors)
	t_r_quat = calc_funcs.vector_to_pure_quaternion(top_right_vectors)
	t_l_quat = calc_funcs.vector_to_pure_quaternion(top_left_vectors)
	rotation_1 = []
	rotation_2 = []
	orientation_final_list = []
	orientation_time = []
	for i in range(len(b_l_quat)):
		rotation_1.append(calc_funcs.hamilton_product(b_r_quat[i], b_l_quat[i]))
	for i in range(len(t_r_quat)):
		rotation_2.append(calc_funcs.hamilton_product(t_r_quat[i], rotation_1[i]))
	for i in range(len(t_l_quat)):
		orientation_final_list.append(calc_funcs.hamilton_product(t_l_quat[i], rotation_2[i]))
